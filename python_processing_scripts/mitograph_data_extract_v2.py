...
def mitograph_data_extract(directory):
	...
	for stage in stages:
		...
		for timepoint in timepoints:
			for time_content in sorted(os.listdir(timepoint)):
				...
				if time_content.endswith('volumes.txt'):
					cell_volumes = timepoint + '/' + time_content
		...
		basepos = stage_df.iloc[1]['image path'].rfind('_s')
		df_volume.loc[:, 'image_path'] = stage_df.iloc[1]['image path'][:basepos] + '_' + df_volume['roi_name']
...
def _main():
	if len(sys.argv) < 1:
		raise Exception ('Usage: mitograph_data_extract_v2.py <directory> <size_of_time_points')
	pathtodir = sys.argv[1]
	mitograph_data_extract(pathtodir)
