...
def mitograph_extended_analysis_carbons(directory, cellID, genotype):
		...
		if test_vol >= maxvol:
			...
			with open((directory + '/' + os.path.basename(directory) + '_morphological_summaries_by_mitochondrion.txt'), 'a') as csv_ind:
				...
				for j in range(threshmitos):
					ind_csv_writer.writerow([surface[:-12], (cellID + stagepos + filename[-16:-13]), genotype, (j + 1), volumes[j], surface_areas[j], NSIs[j], sphericity[j], compacteds[j]])
				csv_ind.close()
			...
			with open((directory + '/' + os.path.basename(directory) + '_morphological_summaries_by_whole_cell.txt'), 'a') as csv_totals:
				total_csv_writer = csv.writer(csv_totals, csv.excel_tab)
				total_csv_writer.writerow([surface[:-12] , (cellID + stagepos + filename[-16:-13]), genotype, threshmitos , total_volume , total_surface_area , total_NSI , total_sphericity, total_length, avgvolume, avgSA, avgNSI, avgsphericity, weighted_avgSA, weighted_avgNSI, weighted_avgsphericity, avgcompacted, weighted_avgcompacted, avg_width, std_width, endpoints, artifacts])
				csv_totals.close()
...
def _main():
	...
	mitograph_extended_analysis_carbons(pathtodir, ID, genotype)
