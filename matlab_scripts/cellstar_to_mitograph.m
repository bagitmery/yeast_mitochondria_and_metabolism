...
function cellstar_to_mitograph(path, timepoints, microns)
...
parfor t = 1:timepoints
    timepoint = ['_t', num2str(t, '%03i'), '_segmentation.mat'];

    % this would save all data in a separate directory for each timepoint
    % to use, comment out lines 25-26
    outdir =  ['t', num2str(t, '%03i')];
    mkdir(path, outdir);
        
    % iterate over all segmented images
    for s = 1:length(segdirectory)
        % if it's indeed segmentation data
        if ~isempty(regexp(segdirectory(s).name, timepoint, 'once'))
            segments = fullfile(segpath, segdirectory(s).name);
            % figure out the stage position, for later
            stagepoint = regexp(segdirectory(s).name, stagereg, 'match');
            stagepoint = char(stagepoint(end));
            % regular expression for the corresponding fluorescence image
            fluorreg = ['w[2-5][^]*_', stagepoint, '_t', num2str(t, '%03i')];
            % iteate over potential fluorescence images
                    ...
                    for c = 1:cells
                        ...
                        if (cellID~=0)
                            ...
                            for z = 1:length(info)
                                ...
                                % expand the mask
                                dilated = imdilate(mask, ones(8,8));
                                ...
                                if z == 1
                                    ...
                                    roi_name = [stagepoint, '_t', num2str(t, '%03i'), '_cell_', num2str(c, '%03i')];
